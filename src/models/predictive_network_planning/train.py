...
from .utils import make_supervised_frame, split_scale, save_feature_meta
...
def main():
    ...
    p.add_argument("--out_dir", default="models/predictive_network_planning")
    ...
    df, X, y, feature_cols = make_supervised_frame(args.csv)
    (Xtr, ytr), (Xv, yv), (Xte, yte), scaler = split_scale(X, y)

    # Save artifacts
    save_feature_meta(args.out_dir, feature_cols)
    joblib.dump(scaler, os.path.join(args.out_dir, "planning_scaler.pkl"))

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = PlanningMLP(input_dim=Xtr.shape[1],
                        hidden=args.hidden,
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(model, tr_loader, opt, loss_fn, device)
        valid_loss = evaluate(model, v_loader, loss_fn, device)
        print(f"Epoch {epoch}: train={train_loss:.4f} | val={valid_loss:.4f}")
