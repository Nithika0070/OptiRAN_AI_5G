...
def test_pnp_model_forward():
    """Test that Predictive Network Planning model produces correct output shape."""
    input_dim = 10
    model = PlanningMLP(input_dim=input_dim)
    x = torch.randn(5, input_dim)
    y = model(x)

    assert y.shape == (5, 1), "PNP model output should have shape (batch_size, 1)"
    assert not torch.isnan(y).any(), "PNP model output contains NaNs"
