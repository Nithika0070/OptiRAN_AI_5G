...
def test_eeo_model_forward():
    """Test that Energy Efficiency Optimization model outputs valid predictions."""
    input_dim = 6
    model = EnergyEfficiencyNet(input_dim=input_dim)
    x = torch.randn(3, input_dim)
    y = model(x)

    assert y.shape == (3, 1), "EEO model output should have shape (batch_size, 1)"
    assert not torch.isnan(y).any(), "EEO model output contains NaNs"
